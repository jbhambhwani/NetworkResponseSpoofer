...
extension NSObject {
    ...
    class func swizzleMethod(originalSelector: Selector, withSelector: Selector) {
        let aClass: AnyClass = object_getClass(self)
        NSObject.swizzleMethod(originalSelector, withSelector: withSelector, forClass: aClass)
    }
    
    private class func swizzleMethod(originalSelector: Selector, withSelector: Selector, forClass: AnyClass) {
