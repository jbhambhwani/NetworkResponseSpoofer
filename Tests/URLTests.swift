...
final class URLTests: XCTestCase {
    ...
    func testSimpleURLNormalization() {
        ...
        XCTAssertTrue(sampleURL1.normalizedString == normalizedSmokeURL,
                      "Normalized version has to have the host and query parameters values stipped away")
    }
    ...
    func testParameterIgnoreURLNormalization() {
        ...
        XCTAssertTrue(complexURL.normalizedString == normalizedComplexURLIgnoringParameters,
                      "Normalized version must match & must ignore specified params")
    }

    func testCapitalURLNormalization() {
        let normalizedAllCapsURL = "jsonplaceholder.typicode.com/users"
        XCTAssertTrue(allCapsURL.normalizedString == normalizedAllCapsURL,
                      "After normalization, all URL's should be lower case")
    }
    ...
    func testPathIgnoreRules() {
        ...
        XCTAssertTrue(complexURL.normalizedString == normalizedPathIgnoredURL,
                      "After normalization, path componets should be ignored if specified")
    }
    ...
    func testPathReplaceRules01() {
        ...
        XCTAssertTrue(sampleURL1.normalizedString == normalizedRangeReplacedURL,
                      "After normalization, ranges should be replaced if specified")
    }
